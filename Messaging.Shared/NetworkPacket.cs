...
public class NetworkPacket : IDisposable
{
    ...
    public static NetworkPacket CreateReadPacket(byte[] buffer)
    {
        return new NetworkPacket(new ReadOnlyMemory<byte>(buffer));
    }

    public static NetworkPacket CreateWritePacket(int size)
    {
        return new NetworkPacket(size);
    }
    ...
    /// <summary>
    /// Returns only data upto write position
    /// </summary>
    /// <param name="totalRead"></param>
    /// <returns></returns>
    public ArraySegment<byte> ToBytesSegment(out int totalRead)
    {
       totalRead = WritePosition;
